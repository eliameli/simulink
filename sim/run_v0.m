...
%
% Русское резюме: главный скрипт проекта. Строит карту, даёт нарисовать
% маршрут мышью, строит эталонную траекторию, собирает и запускает
% Simulink-модель, потом строит графики сравнения.
...
map = generate_map();   % строим синтетическую карту дорог (граф узлы/рёбра)
...
waypoints = pick_route(map);   % даём пользователю нарисовать маршрут мышью на карте
v_cruise  = 8;     % m/s, constant cruise speed (~29 km/h)
% ^ м/с - постоянная скорость движения (~29 км/ч)
dt        = 0.01;  % s, fixed step - used for both the truth generation
                    % and the Simulink model, they must match
% ^ с - фиксированный шаг времени (одинаковый и для эталона, и для модели)
truth = generate_trajectory(waypoints, v_cruise, dt);   % строим эталонную (истинную) траекторию по маршруту
...
mdl = 'gps_free_nav_v0';   % имя Simulink-модели
...
build_model(dt, params, mdl_path); % программно строим Simulink-модель заново
...
% ^ начальная координата x для блока dead reckoning (берём из эталона)
...
set_param(mdl, 'StopTime', num2str(truth.t(end)));   % задаём длительность симуляции = длительности маршрута
...
figure('Name', 'GPS-free navigation v0 - trajectory');   % новое окно с графиком траекторий
...
% ^ чёрная линия - истинная (эталонная) траектория
...
title('True vs. dead-reckoned vs. map-matched trajectory'); % заголовок графика

figure('Name', 'GPS-free navigation v0 - error');   % новое окно с графиком ошибки
...
figure('Name', 'GPS-free navigation v0 - diagnostics');   % новое окно с диагностическими графиками
